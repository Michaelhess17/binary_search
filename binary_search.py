...
def count_repeats(xs, x):
    ...
    def go_one(left, right):
        '''
        Recursive function that finds the smallest index of xs
        where xs[index] >= x
        '''
        if (left == 0) & (xs[left] == x):
            return left
    ...
    right_ind = go_two(0, len(xs) - 1)
    left_ind = go_one(0, len(xs) - 1)
    print("[Init] right_ind, left_ind=", right_ind, left_ind)
    if (right_ind == -1) & (left_ind == -1):
        print("right_ind, left_ind=", right_ind, left_ind)
        return 0
    elif (right_ind == -1) or (left_ind == -1):
        print("right_ind, left_ind=", right_ind, left_ind)
        return 1
    return 1 + right_ind - left_ind
...
def argmin(f, lo, hi, epsilon=1e-3):
    ...
    The basic algorithm is:
        ...
        2) For each recursive call:
            ...
            depending on which one is the smallest; you recursively call
            your function on the interval [lo,m2] or [m1,hi]

    APPLICATION:
    Essentially all data mining algorithms are just this argmin
    implementation in disguise. If you go on to take the data mining
    class (CS145/MATH166), we will spend a lot of time talking about different
    f functions that can be minimized and their applications.
    But the actual minimization code will be a variant of this binary search.
    ...
    due to minor implementation details. Writing tests for code
    that uses floating point numbers is notoriously difficult.
    ...
    '''
    if hi - lo < epsilon:
        # print("hi, lo, f(hi), f(lo)=", hi, lo, f(hi), f(lo))
        return lo
    else:
        m1, m2 = (lo + ((hi - lo) / 3)), (lo + ((2 * (hi - lo)) / 3))
        current_lo = 1e100
        current_lo_pnt = lo
        for pnt in [lo, m1, m2, hi]:
            if f(pnt) < current_lo:
                current_lo = f(pnt)
                current_lo_pnt = pnt
        if (current_lo_pnt == lo) or (current_lo_pnt == m1):
            print('Case 1: \n')
            print("lo, m2, f(lo), f(m2)=", lo, m2, f(lo), f(m2))
